...
$c->add_dataset_field(
	"eprint",
	{ name => "rioxx2_date_accepted", type => "date", min_resolution => "month", required => 1 }
);

$c->add_dataset_field(
	"eprint",
	{ name => "rioxx2_freetoread", type => "compound", fields => [
		{ sub_name => "start", type => "date" },
		{ sub_name => "end", type => "date" }
	] }
);

$c->add_dataset_field(
	"eprint",
	{ name => "rioxx2_license", type => "compound", fields => [
		{ sub_name => "url", type => "url" },
		{ sub_name => "start", type => "date" }
	], required => 1 }
);
...
$c->add_dataset_field(
	"eprint",
	{ name => "rioxx2_type", type => "set",
	options => [qw( book book_chapter book_edited conference_item journal_article manual monograph policy_report tech_report tech_standard thesis other consultancy_report working_paper 	)], required => 1, multiple => 0 }# N.B. Profile says this is a "one or more"
);
